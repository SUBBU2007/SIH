...
export function computeScore(nutritionPer100) {
  ...
  const sugarScore = Math.max(
    0,
    100 - nutritionPer100.sugar_g * 3
  );

  const sodiumScore = Math.max(
    0,
    100 - nutritionPer100.sodium_mg / 8
  );

  const proteinScore = Math.min(
    100,
    nutritionPer100.protein_g * 8
  );

  const satFatScore = Math.max(
    0,
    100 - nutritionPer100.sat_fat_g * 6
  );
...
export function checkMismatch(
  ...
  const severity = fields.some(
    (field) => field.differencePercent > 20
  )
    ? "high"
    : fields.length
      ? "warning"
